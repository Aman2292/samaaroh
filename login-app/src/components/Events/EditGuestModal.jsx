...
const EditGuestModal = ({ eventId, guest, isOpen, onClose, onSuccess }) => {
    ...
    // Pre-fill form when guest data changes
    useEffect(() => {
        if (guest) {
            setFormData({
                name: guest.name || '',
                ...
                side: guest.side || '',
                group: guest.group || '',
                rsvpStatus: guest.rsvpStatus || 'invited',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`http://localhost:5001/api/events/${eventId}/guests/${guest._id}`, {
                method: 'PUT',
                headers: {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <div className="sticky top-0 bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between">
                    <div>
                        <h2 className="text-2xl font-bold text-slate-800">Edit Guest</h2>
                        {guest.addedOnsite && (
